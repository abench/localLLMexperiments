...
def write_results_to_csv(fname:str, data, header):
    """
    Store csv file to disk
    :param fname: name of file (string)
    :param data: data to save (list of dictionaries)
    :param header: csv file header (list of field names)
    :return: None
    """
...
loader = UnstructuredURLLoader(url_list_small)
docs = loader.load()
...
Give output as Python dictionary with following keys:
...
#
# Prepare to summarization chain
#
...
result_fields_names = ["date","space_ship_name","COSPAR ID", "astronauts_name","summary","source"]
results_table = []
...
for i, doc in enumerate(docs):
    ...
    sleep(5)
write_results_to_csv("output_3.csv", results_table, result_fields_names)
